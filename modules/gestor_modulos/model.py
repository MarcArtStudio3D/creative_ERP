...
class RolePermissionsManager:
    ...
    def set_module_permissions(
        self, role: str, module_id: str, perms: List[str]
    ) -> None:
        """
        ...
        normalized_perms = [
            p.strip().upper()
            for p in perms
            if p.strip().upper() in self.VALID_PERMISSIONS
        ]
    ...
    def set_multiple_modules_permissions(
        self, role: str, module_ids: List[str], perms: List[str]
    ) -> int:
        """
