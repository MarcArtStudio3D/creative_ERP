...
def test_view_save_includes_family():
    ...
    sub = session.execute(
        text(
            "SELECT id, subfamilia FROM subfamilias WHERE id_familia = :fid ORDER BY id LIMIT 1"
        ),
        {"fid": familia_id},
    ).fetchone()
    if sub:
    ...
    view.ui.txtdescripcionResumida.setText(
        controller.get_current_article().get("descripcion_reducida", "Test")
    )
    ...
    if sub_id:
        ...
        print(
            "ℹ️ No había subfamilias para esta familia en la BD, omitiendo comprobación de subfamilia"
        )
