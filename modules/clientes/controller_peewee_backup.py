...
class ClientesController(QObject):
    ...
    def create_cliente(self, data: Dict) -> Optional[Cliente]:
    ...
    def update_cliente(self, id_cliente: int, data: Dict) -> Optional[Cliente]:
    ...
    def save_current_cliente_with_flags(self, force_create: bool = False, force_update: bool = False) -> bool:
        ...
        try:
            ...
            data = self.repository.model_to_dict(self._current_cliente)
            cid = getattr(self._current_cliente, 'id', None)
    ...
    def delete_cliente(self, id_cliente: int) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.data_changed.emit()
                if self._current_cliente and getattr(self._current_cliente, 'id', None) == id_cliente:
    ...
    def next_cliente(self) -> bool:
        ...
        try:
            ...
            next_cliente = self.repository.get_next(int(cid))
            if next_cliente:
                self._current_cliente = next_cliente
                self.cliente_changed.emit(int(getattr(next_cliente, 'id')))
                return True
            return False
        except Exception as e:
    ...
    def prev_cliente(self) -> bool:
        ...
        try:
            ...
            prev_cliente = self.repository.get_prev(int(cid))
            if prev_cliente:
                self._current_cliente = prev_cliente
                self.cliente_changed.emit(int(getattr(prev_cliente, 'id')))
                return True
            return False
        except Exception as e:
    ...
    def cargar_clientes(self, filtro: str = ""):
        ...
        try:
            ...
            for cliente in clientes:
                # Obtener valores de forma segura desde la instancia
                codigo = getattr(cliente, 'codigo_cliente', '') or ''
                nif = getattr(cliente, 'cif_nif_siren', '') or ''
                nombre = getattr(cliente, 'nombre_fiscal', '') or ''
                telefono = getattr(cliente, 'telefono1', '') or ''
