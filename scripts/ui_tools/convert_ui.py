    ...
    try:
        ...
        for line in lines:
            ...
            if (
                ...
                content = line.strip()
                new_lines.append(
                    f"{indent}# {content} # Desactivado por convert_ui.py para usar tema global\n"
                )
                modified = True
            else:
        ...
        print(
            f"  ⚠ Advertencia: No se pudieron limpiar estilos en {file_path.name}: {e}"
        )
    ...
    try:
        while True:
            ...
            for ui_file in ui_files:
                ...
                if (
                    ui_file not in file_timestamps
                    or file_timestamps[ui_file] != current_mtime
                ):
