    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            try:
                ...
                connection.execute(
                    text(
                        "ALTER TABLE clientes MODIFY COLUMN id_pais VARCHAR(100) DEFAULT 'España'"
                    )
                )
            ...
            try:
                result = connection.execute(
                    text("UPDATE clientes SET id_pais = 'España' WHERE id_pais = '1'")
                )
                print(
                    f"   ✅ Registros actualizados ('1' -> 'España'): {result.rowcount}"
                )
            except Exception as e:
