...
from core.db import get_engine_for_database, get_engine_from_url, get_session
...
def drop_table_if_exists(engine, table_name: str):
    ...
    if table_name in inspector.get_table_names():
        logging.getLogger(__name__).info(
            f"  ✓ Tabla '{table_name}' encontrada, eliminando..."
        )
        with engine.connect() as conn:
            conn.execute(text(f"DROP TABLE {table_name}"))
            conn.commit()
        logging.getLogger(__name__).info(
            f"  ✅ Tabla '{table_name}' eliminada correctamente"
        )
    else:
        logging.getLogger(__name__).info(
            f"  ℹ️  Tabla '{table_name}' no existe, nada que hacer"
        )
...
def main():
    ...
    logging.getLogger(__name__).info(
        "Eliminando tabla 'direcciones_alternativas_empresas'"
    )
    logging.getLogger(__name__).info("=" * 60)
            ...
            for empresa in empresas:
                ...
                try:
                    ...
                    drop_table_if_exists(
                        empresa_engine, "direcciones_alternativas_empresas"
                    )
                    try:
                    ...
                    logging.getLogger(__name__).exception(
                        f"    ❌ Error procesando empresa {empresa.id}"
                    )
