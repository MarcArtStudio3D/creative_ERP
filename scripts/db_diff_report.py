...
from core.db import DATABASE_CONFIGS, get_engine, set_current_database
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare configured DBs and produce table/row counts + sample rows"
    )
    parser.add_argument(
        "--databases",
        nargs="+",
        default=list(DATABASE_CONFIGS.keys()),
        help="Database keys to inspect (default: all configured)",
    )
    parser.add_argument(
        "--sample",
        type=int,
        default=0,
        help="Number of sample rows per suspicious table",
    )
    parser.add_argument(
        "--output-csv",
        help="Optional directory to dump CSV samples for inspected tables",
    )
    ...
    if not keys:
        print(
            "No valid databases selected. Available keys:",
    ...
    if len(keys) >= 2:
        ...
        for other in keys[1:]:
            ...
            common = sorted(
                list(all_tables_sets[base].intersection(all_tables_sets[other]))
            )
            diffs = []
            for t in common:
        ...
        if suspicious:
            ...
            for t in suspicious:
                ...
                if rows:
                    for r in rows:
                        print(
                            " ", {k: v for k, v in r.items() if k in list(r.keys())[:6]}
                        )
                else:
