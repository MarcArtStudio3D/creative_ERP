...
def clean_hardcoded_colors(content):
    ...
    content = re.sub(r'"background-color: rgb\(\d+,\s*\d+,\s*\d+\);"', '""', content)
    ...
    content = re.sub(
        r'\.setStyleSheet\(u"color: rgb\(0, 0, 0\);"\)', '.setStyleSheet(u"")', content
    )
    ...
    while i < len(lines):
        ...
        if "setStyleSheet" in line and "background-color: rgb(" in line:
            ...
            if "alternate-background-color" in line:
                ...
                new_line = re.sub(
                    r"background-color: rgb\(\d+,\s*\d+,\s*\d+\);?\s*", "", line
                )
                ...
                if line.strip().endswith('\\n"') and i + 1 < len(lines):
                    next_line = lines[i + 1]
                    if (
                        "background-color: rgb(" in next_line
...
def main():
    if len(sys.argv) != 2:
        logging.getLogger(__name__).error(
            "Usage: python clean_ui_colors.py <ui_python_file>"
        )
        sys.exit(1)
