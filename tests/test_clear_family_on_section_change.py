...
def test_clear_family_on_section_change():
    ...
    art = session.execute(text("SELECT id, id_seccion, id_familia, id_subfamilia FROM articulos WHERE id_familia IS NOT NULL LIMIT 1")).fetchone()
    if not art:
    ...
    print(f"Artículo de prueba: id={art_id}, seccion={orig_seccion}, familia={orig_familia}, sub={orig_sub}")
    ...
    sec = session.execute(text("SELECT id, codigo, seccion FROM secciones WHERE id != :id LIMIT 1"), {'id': orig_seccion}).fetchone()
    if not sec:
    ...
    db_row = session.execute(text("SELECT id_familia, id_subfamilia FROM articulos WHERE id = :id"), {'id': art_id}).fetchone()
    print('Fila en BD tras guardado:', db_row)

    assert db_row is not None
    assert db_row[0] is None, 'id_familia debería quedar NULL tras cambiar de sección y guardar'
    assert db_row[1] is None, 'id_subfamilia debería quedar NULL tras cambiar de sección y guardar'
