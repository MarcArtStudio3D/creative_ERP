...
def test_subfamily_button_conditions():
    ...
    session = get_session()
    art = session.execute(text("SELECT id FROM articulos WHERE id_familia IS NULL LIMIT 1")).fetchone()
    if not art:
    ...
    fam = session.execute(text("SELECT id, codigo, familia FROM familias LIMIT 1")).fetchone()
    if not fam:
