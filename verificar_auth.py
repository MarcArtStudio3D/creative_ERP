...
from core.auth import UserRole, get_role_permissions
...
def verificar_auth():
    ...
    if "divisiones_almacen" in perms:
        logging.getLogger(__name__).info(
            "   ✅ 'divisiones_almacen' encontrado en permisos de ADMIN"
        )
        logging.getLogger(__name__).debug(
            f"   Permissions: {[p.value for p in perms['divisiones_almacen']]}"
        )
    else:
        logging.getLogger(__name__).warning(
            "   ❌ 'divisiones_almacen' NO encontrado en permisos de ADMIN"
        )
        return False
    ...
    if "divisiones_almacen" in perms:
        logging.getLogger(__name__).info(
            "   ✅ 'divisiones_almacen' encontrado en permisos de EMPLOYEE"
        )
        logging.getLogger(__name__).debug(
            f"   Permissions: {[p.value for p in perms['divisiones_almacen']]}"
        )
        ...
        logging.getLogger(__name__).warning(
            "   ❌ 'divisiones_almacen' NO encontrado en permisos de EMPLOYEE"
        )
        return False
    ...
    logging.getLogger(__name__).info("=" * 60)
    logging.getLogger(__name__).info(
        "El sistema de autenticación ahora lee correctamente los permisos"
    )
