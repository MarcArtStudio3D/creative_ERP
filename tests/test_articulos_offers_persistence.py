...
def test_offers_persistence_roundtrip():
    ...
    if oferta is None:
        rows = session.execute(
            text(
                "SELECT id, id_articulo, id_tarifa, fecha_inicio, fecha_fin, activa FROM articulos_ofertas WHERE id_articulo = :id"
            ),
            {"id": art_id},
        ).fetchall()
        ...
        if rows:
            ...
            print(
                "DEBUG: No oferta found after controller.save — attempting manual upsert for diagnostics"
            )
            manual_payload = {
            ...
            assert (
                oferta is not None
            ), "Manual upsert succeeded but repository.get_oferta_for_article still returns None"
