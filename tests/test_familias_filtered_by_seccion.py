...
def test_familias_filtered_by_seccion():
    ...
    rows = session.execute(text("SELECT id FROM secciones ORDER BY id LIMIT 2")).fetchall()
    if len(rows) < 2:
    ...
    fams1 = session.execute(text("SELECT id FROM familias WHERE id_seccion = :sid ORDER BY id"), {'sid': sec1}).fetchall()
    fams2 = session.execute(text("SELECT id FROM familias WHERE id_seccion = :sid ORDER BY id"), {'sid': sec2}).fetchall()
    ...
    ctrl_ids2 = set([f['id'] for f in controller_f2])

    assert repo_ids1 == ctrl_ids1, 'Controller debe delegar en el repo y devolver los mismos ids'

    # Asegurarse que los sets no se mezclan
    if repo_ids1 and repo_ids2:
        assert repo_ids1.isdisjoint(repo_ids2), 'Familias de diferentes secciones no deben mezclarse'
