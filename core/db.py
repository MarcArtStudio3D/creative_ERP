...
from sqlalchemy.sql.sqltypes import Date, DateTime, Float, Integer, String, Text
...
def set_current_database(db_name):
    ...
    if db_name not in DATABASE_CONFIGS:
        raise ValueError(
            f"Base de datos '{db_name}' no configurada. Opciones disponibles: {list(DATABASE_CONFIGS.keys())}"
        )
...
def init_db(db_name=None, initiator: str | None = None):
    ...
    logger.info(
        f"init_db requested for database: {get_current_database()} by {initiator}"
    )
    if get_current_database() == "main":
        ...
        get_current_database() == "artstudio3d"
        or _is_company_database_pointing_to_artstudio3d()
    ):
            ...
            try:
                with engine.connect() as conn:
                    ...
                    logging.getLogger(__name__).info(
                        "Column added: %s.%s (%s)", table_name, col_name, sql_type
                    )
            except Exception as e:
                # best-effort: ignore failures to avoid breaking startup
                logging.getLogger(__name__).warning(
                    "Error adding column %s.%s: %s", table_name, col_name, e
                )
                continue


# Funciones para gestión dinámica de bases de datos por empresa
def set_database_for_company(
    company_id: int, init: bool = False, initiator: str | None = None
):
    """
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "Database switched for company %s", company_id
        )
        ...
        if init:
            logger = logging.getLogger("core.db.init")
            logger.info(
                f"init_db will run for company_{company_id} requested by {initiator}"
            )
            init_db(initiator=initiator)

    except Exception as e:
        logging.getLogger(__name__).exception(
            "ERROR configuring database for company %s: %s", company_id, e
        )
        raise
    ...
    try:
        ...
        if db_url.startswith("mysql+pymysql://"):
            ...
            for driver in mysql_drivers:
                if driver in available_drivers:
                    ...
                    if db.open():
                        logging.getLogger(__name__).info(
                            "QSqlDatabase connection created (%s): %s -> %s",
                            driver,
                            connection_name,
                            database,
                        )
            ...
            logging.getLogger(__name__).warning(
                "MySQL not available, using SQLite fallback for queries..."
            )
            return _create_sqlite_fallback(connection_name)

        # PostgreSQL
        elif db_url.startswith("postgresql://") or db_url.startswith(
            "postgresql+psycopg2://"
        ):
            ...
            if "QPSQL" in QSqlDatabase.drivers():
                ...
                if db.open():
                    logging.getLogger(__name__).info(
                        "QSqlDatabase connection created (QPSQL): %s -> %s",
                        connection_name,
                        database,
                    )
                    return db
                    ...
                    logging.getLogger(__name__).warning(
                        "PostgreSQL connection failed: %s", db.lastError().text()
                    )
                    QSqlDatabase.removeDatabase(connection_name)

            # Fallback a SQLite para PostgreSQL
            logging.getLogger(__name__).warning(
                "PostgreSQL not available, using SQLite fallback for queries..."
            )
            return _create_sqlite_fallback(connection_name)
        ...
        # SQLite
            ...
            if "QSQLITE" in QSqlDatabase.drivers():
                ...
                if db.open():
                    logging.getLogger(__name__).info(
                        "QSqlDatabase connection created (QSQLITE): %s -> %s",
                        connection_name,
                        db_path,
                    )
                    return db
                    ...
                    logging.getLogger(__name__).warning(
                        "Error opening SQLite connection: %s", db.lastError().text()
                    )
                    QSqlDatabase.removeDatabase(connection_name)
...
def _create_sqlite_fallback(connection_name: str):
    ...
    try:
        ...
        if "QSQLITE" in QSqlDatabase.drivers():
            ...
            if db.open():
                logging.getLogger(__name__).info(
                    "SQLite temporal creado: %s", temp_path
                )
...
def populate_sqlite_temp_with_data(qt_db, table_name: str, data: list, columns: list):
    ...
    try:
        ...
        if not query.exec(f"CREATE TABLE {table_name} ({columns_sql})"):
            logging.getLogger(__name__).warning(
                "Error creando tabla temporal: %s", query.lastError().text()
            )
            return False
        ...
        for row_data in data:
            ...
            if not query.exec():
                logging.getLogger(__name__).warning(
                    "Error insertando fila: %s", query.lastError().text()
                )
                continue

        logging.getLogger(__name__).info(
            "Tabla temporal %s poblada con %d registros", table_name, len(data)
        )
        return True
