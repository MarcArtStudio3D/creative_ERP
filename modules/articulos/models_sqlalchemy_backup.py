...
class Articulo(Base):
    ...
    id_tipo: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("articulo_tipo.id"), nullable=True
    )
    ...
    porc_dto: Mapped[float] = mapped_column(
        Float, default=0.0
    ...
    fecha_prevista_recepcion: Mapped[Optional[date]] = mapped_column(
        Date, nullable=True
    )
...
class Tarifa(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    id_articulo: Mapped[int] = mapped_column(
        Integer, ForeignKey("articulos.id"), nullable=False
    )
...
class ProveedorFrecuente(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    id_art: Mapped[int] = mapped_column(
        Integer, ForeignKey("articulos.id"), nullable=False
    )
    ...
    pvd_real: Mapped[float] = mapped_column(
        Float, default=0.0
    )  # PVD real con descuentos
...
class ArticuloOferta(Base):
    ...
    oferta_precio_final: Mapped[bool] = mapped_column(
    ...
    regalo: Mapped[float] = mapped_column(
        Float, default=0.0
    )  # Para 3x2: lleva Y gratis
...
class ArticuloImagen(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    id_articulo: Mapped[int] = mapped_column(
        Integer, ForeignKey("articulos.id"), unique=True, nullable=False
    )
...
class Kit(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    codigo_kit: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # Código del artículo kit
