...
class Cliente(Base):
    ...
    # Datos financieros
    porc_dto_cliente: Mapped[float] = mapped_column(
        Float, default=0.0
    )  # Porcentaje descuento fijo
    recargo_equivalencia: Mapped[bool] = mapped_column(Boolean, default=False)
    irpf: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # Cliente empresa (aplicar IRPF)
    grupo_iva: Mapped[int] = mapped_column(
        Integer, default=1
    )  # 1=General, 2=UE, 3=Exento, 4=Exportación
    ...
    id_tarifa: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Tarifa de precios
    id_divisa: Mapped[int] = mapped_column(Integer, default=1)  # Divisa predeterminada
    id_idioma_documentos: Mapped[int] = mapped_column(
        Integer, default=1
    )  # Idioma para documentos
    id_agente: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Agente comercial
    id_transportista: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
...
class DireccionAlternativa(Base):
    ...
    descripcion: Mapped[Optional[str]] = mapped_column(
        String(100)
    )  # Ej: "Almacén principal", "Oficina central"
    direccion1: Mapped[Optional[str]] = mapped_column(String(255))
    direccion2: Mapped[Optional[str]] = mapped_column(String(255))
    cp: Mapped[Optional[str]] = mapped_column(String(10))
    poblacion: Mapped[Optional[str]] = mapped_column(String(100))
...
class DeudaCliente(Base):
    ...
    id_documento: Mapped[Optional[int]] = mapped_column(
        Integer
    )  # ID del documento (factura, ticket, etc.)
    tipo_documento: Mapped[Optional[str]] = mapped_column(
        String(20)
    )  # 'factura', 'ticket', 'albaran'
...
class HistorialCliente(Base):
    ...
    tipo_operacion: Mapped[Optional[str]] = mapped_column(
        String(50)
    )  # 'venta', 'cobro', 'devolucion', 'nota'
...
class ClienteTipo(Base):
    ...
    id_tipo: Mapped[int] = mapped_column(
        Integer, ForeignKey("tipocliente_def.id"), nullable=False
    )
    id_subtipo: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("tiposubcliente_def.id"), nullable=True
    )
