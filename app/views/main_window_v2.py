...
from PySide6.QtCore import QDate, QEasingCurve, QPoint, QPropertyAnimation, Qt, Signal
...
from core.module_manager import ModuleCategory, ModuleManager, Permission
...
class MainWindowV2(QMainWindow):
    ...
    def setup_ui(self) -> None:
        ...
        content_area_layout.addWidget(
            self.stacked_widget, 1
        )  # stretch=1 para que ocupe el espacio restante
    ...
    def create_sidebar(self) -> QFrame:
        ...
        self.sidebar_modules_container.setContentsMargins(
            8, 10, 12, 5
        )  # Más margen derecho para scrollbar
    ...
    def update_sidebar_modules(self, category: Optional[ModuleCategory] = None) -> None:
        ...
        for cat in [
            ...
            self.sidebar_modules_container.insertWidget(
                self.sidebar_modules_container.count() - 1, card
            )

            # Espaciado entre tarjetas
            self.sidebar_modules_container.insertSpacing(
                self.sidebar_modules_container.count() - 1, 12
            )
    ...
    def _create_category_card(
        ...
        card.mousePressEvent = lambda event: self.show_category_modules(
            category, modules
        )
    ...
    def create_top_bar(self) -> QFrame:
        ...
        self.user_label.setStyleSheet(
            "color: rgb(255, 255, 127); background-color: transparent;"
        )
        ...
        self.year_selector.setStyleSheet(
            "color: rgb(255, 255, 127); background-color: rgb(30, 30, 30);"
        )
    ...
    def create_welcome_page(self) -> QWidget:
        ...
        instructions = QLabel(
            self.tr("Selecciona un módulo del menú superior para comenzar")
        )
    ...
    def create_menus(self) -> None:
        ...
        try:
            if (
                self.session.has_permission("empresas", Permission.ADMIN)
                or self.session.user.role == UserRole.ADMIN
            ):
                init_company_action = QAction(
                    self.tr("Inicializar BD de Empresa..."), self
                )
                init_company_action.setStatusTip(
                    self.tr(
                        "Inicializa el esquema de base de datos para una empresa (acción admin)"
                    )
                )
                init_company_action.triggered.connect(
                    lambda *_: self.open_admin_init_dialog()
                )
    ...
    def create_global_avisos_panel(self) -> QWidget:
        ...
        bg_color_rgb = (
            (200, 50, 50) if has_avisos else (70, 130, 180)
        )  # Rojo si hay avisos, azul si no
        ...
        class VerticalTabButton(QWidget):
            ...
            def paintEvent(self, event):
                ...
                path.arcTo(
                    rect.width() - radius * 2, 0, radius * 2, radius * 2, 90, -90
                )
        ...
        tab = VerticalTabButton(
            self.tr("AVISOS"), bg_color_rgb, border_color_rgb, hover_color_rgb
        )
        ...
        def toggle_panel() -> None:
            if getattr(panel, "_is_open", False):
                ...
                getattr(container, "_animation").setEndValue(
                    QPoint(-230, container.pos().y())
                )
    ...
    def open_module(self, module_id: str) -> None:
        ...
        if module_id in self.module_widgets:
            ...
            if idx != -1:
                ...
                if (
                    module_view is None
                    or not getattr(module_view, "isVisible", lambda: True)()
                ):
        ...
        if module_widget:
            ...
            print(
                f"Módulos en caché: {len(self.module_widgets)}/{self.max_cached_modules}"
            )
        else:
            show_info(
                self,
    ...
    def _cleanup_old_modules(self) -> None:
        ...
        if oldest_module_id in self.module_widgets:
            ...
            print(
                f"Módulo '{oldest_module_id}' eliminado de caché para liberar memoria"
            )
            print(
                f"Módulos restantes en caché: {len(self.module_widgets)}/{self.max_cached_modules}"
            )
    ...
    def clear_module_cache(self) -> None:
        ...
        modules_to_remove = [
            mid for mid in self.module_widgets.keys() if mid != current_module_id
        ]
        ...
        print(
            f"Caché de módulos limpiada. Módulos eliminados: {len(modules_to_remove)}"
        )
    ...
    def create_module_widget(self, module_id: str) -> Optional[QWidget]:
            ...
            if ui_obj is not None:
                ...
                if close_btn is not None:
                    ...
                    close_btn.clicked.connect(
                        lambda checked=False, mid=module_id: self.close_module(mid)
                    )
        except Exception:
            pass
        ...
        actions_panel = self.create_module_side_panel(
            module_id, module_info, module_view=module_content
        )
    ...
    def load_module_view(self, module_id: str) -> Optional[QWidget]:
        ...
        try:
            ...
            print(
                f"Intentando cargar módulo {module_id} desde {module_name}.{view_class_name}"
            )
    ...
    def create_placeholder_content(self, module_info) -> QWidget:
        ...
        header.setStyleSheet(
            "background-color: palette(light); border-bottom: 1px solid palette(mid);"
        )
        header_layout = QHBoxLayout()
    ...
    def create_module_side_panel(
        self, module_id: str, module_info, module_view=None
    ) -> QWidget:
        """
        ...
        def toggle_panel() -> None:
            ...
            if getattr(panel, "_is_open", False):
                ...
                getattr(container, "_animation").setEndValue(
                    QPoint(parent.width() - 20, container.pos().y())
                )
                getattr(container, "_animation").start()
                setattr(panel, "_is_open", False)
            else:
                ...
                getattr(container, "_animation").setEndValue(
                    QPoint(parent.width() - 250, container.pos().y())
                )
                getattr(container, "_animation").start()
                setattr(panel, "_is_open", True)
    ...
    def get_module_actions(self, module_id: str) -> list:
        ...
        common_actions = {
            "facturas": [
                {
                    "icon": ":/PNG/resources/icons/png/Add.png",
                    "label": self.tr("Nueva"),
                    "action": "new",
                    "tooltip": self.tr("Crear nueva factura"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/search.png",
                    "label": self.tr("Buscar"),
                    "action": "search",
                    "tooltip": self.tr("Buscar facturas"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Listado"),
                    "action": "list",
                    "tooltip": self.tr("Ver listado completo"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/Print.png",
                    "label": self.tr("Imprimir"),
                    "action": "print",
                    "tooltip": self.tr("Imprimir factura"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/Save.png",
                    "label": self.tr("Exportar"),
                    "action": "export",
                    "tooltip": self.tr("Exportar XML/PDF"),
                },
            ],
            "clientes": [
                {
                    ...
                    "label": self.tr("Nuevo"),
                    "action": "new",
                    "tooltip": self.tr("Crear nuevo cliente"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/search.png",
                    "label": self.tr("Buscar"),
                    "action": "search",
                    "tooltip": self.tr("Buscar clientes"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Listado"),
                    "action": "list",
                    "tooltip": self.tr("Ver todos los clientes"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Estadísticas"),
                    "action": "stats",
                    "tooltip": self.tr("Estadísticas de clientes"),
                },
            ],
            "productos": [
                {
                    ...
                    "label": self.tr("Nuevo"),
                    "action": "new",
                    "tooltip": self.tr("Crear nuevo producto"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/search.png",
                    "label": self.tr("Buscar"),
                    "action": "search",
                    "tooltip": self.tr("Buscar productos"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Inventario"),
                    "action": "inventory",
                    "tooltip": self.tr("Ver inventario"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Categorías"),
                    "action": "categories",
                    "tooltip": self.tr("Gestionar categorías"),
                },
            ],
            "proyectos": [
                {
                    ...
                    "label": self.tr("Nuevo"),
                    "action": "new",
                    "tooltip": self.tr("Crear nuevo proyecto"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/List.png",
                    "label": self.tr("Dashboard"),
                    "action": "dashboard",
                    "tooltip": self.tr("Panel de proyectos"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/Calendar.png",
                    "label": self.tr("Planificación"),
                    "action": "planning",
                    "tooltip": self.tr("Planificar tareas"),
                },
                {
                    "icon": ":/PNG/resources/icons/png/Pay.png",
                    "label": self.tr("Presupuestos"),
                    "action": "budgets",
                    "tooltip": self.tr("Gestionar presupuestos"),
                },
            ],
        }

        # Retornar acciones específicas o genéricas
        return common_actions.get(
            module_id,
            [
                {
                    "icon": ":/PNG/resources/icons/png/Add.png",
    ...
    def _find_module_view(self, container: QWidget) -> Optional[QWidget]:
        ...
        for child in container.findChildren(QWidget):
            ...
            if (
                hasattr(child, "nuevo_cliente")
                or hasattr(child, "nuevo")
                or hasattr(child, "editar_cliente")
                or hasattr(child, "editar")
                or hasattr(child, "borrar_cliente")
                or hasattr(child, "borrar")
            ):
                return child
    ...
    def on_search_changed(
        self, module_id: str, search_text: str, order_by: str, order_mode: str
    ) -> None:
    ...
    def on_module_action(self, module_id: str, action: str) -> None:
        ...
        if action == "refresh":
            ...
            show_info(
                self,
                self.tr("Refrescar"),
                f"{self.tr('Actualizando datos de')} {module_id}...",
            )
            ...
            if not module_view:
                show_warning(
                    self,
                    self.tr("Error"),
                    self.tr("No se pudo encontrar la vista del módulo"),
                )
                return

            # Llamar al método correspondiente según la acción
            if action == "new":
                self._call_module_method(
                    module_view,
                    ["nuevo_cliente", "nuevo", "nuevo_registro", "on_nuevo_cliente"],
                )
                ...
                self._call_module_method(
                    module_view,
                    ["editar_cliente", "editar", "editar_registro", "on_edit_cliente"],
                )
                # Cerrar panel lateral para dar espacio a la edición
                self._close_side_panel(module_id)
            elif action == "delete":
                ...
                show_info(
                    self, self.tr("Excepciones"), self.tr("Funcionalidad en desarrollo")
                )
    ...
    def change_company(self) -> None:
        ...
        if os.environ.get("PYTEST_CURRENT_TEST"):
            ...
            msg.addButton(self.tr("Sí"), QMessageBox.ButtonRole.YesRole)
        ...
        if reply == QMessageBox.StandardButton.Yes or (
            hasattr(reply, "role")
            and getattr(reply, "role", None) == QMessageBox.ButtonRole.YesRole
        ):
    ...
    def open_preferences(self) -> None:
        ...
        show_info(
            self,
            self.tr("Preferencias"),
            self.tr("Ventana de preferencias en desarrollo"),
        )
    ...
    def show_about(self) -> None:
        ...
        show_info(
            ...
            self.tr(
                "<h2>Creative ERP</h2><p>Sistema de Gestión Empresarial</p>"
    ...
    def closeEvent(self, event) -> None:
        ...
        for module_id, widget in self.module_widgets.items():
            ...
            if hasattr(widget, "has_unsaved_changes") and callable(
                getattr(widget, "has_unsaved_changes")
            ):
            ...
            # Para módulos que están en un contenedor, buscar el widget interno
                ...
                for child in widget.findChildren(QWidget):
                    if hasattr(child, "has_unsaved_changes") and callable(
                        getattr(child, "has_unsaved_changes")
                    ):
        ...
        if modules_with_changes:
            ...
            if os.environ.get("PYTEST_CURRENT_TEST"):
                reply = show_question(
                    ...
                    (
                        self.tr("Hay cambios sin guardar en {} módulos").format(
                            len(modules_with_changes)
                        )
                        if len(modules_with_changes) > 1
                        else self.tr("Hay cambios sin guardar en el módulo: {}").format(
                ...
                if len(modules_with_changes) == 1:
                    msg.setText(
                        self.tr("Hay cambios sin guardar en el módulo: {}").format(
                            modules_with_changes[0]
                        )
                    )
                else:
                ...
                save_btn = msg.addButton(
                    self.tr("Guardar"), QMessageBox.ButtonRole.AcceptRole
                )
                discard_btn = msg.addButton(
                    self.tr("Descartar"), QMessageBox.ButtonRole.DestructiveRole
                )
                ...
                if clicked_button == save_btn:
                    ...
                    for module_id in modules_with_changes:
                        ...
                        try:
                            ...
                            if hasattr(widget, "_save_changes") and callable(
                                getattr(widget, "_save_changes")
                            ):
                            ...
                            # Buscar en widgets hijos
                                ...
                                for child in widget.findChildren(QWidget):
                                    if hasattr(child, "_save_changes") and callable(
                                        getattr(child, "_save_changes")
                                    ):
