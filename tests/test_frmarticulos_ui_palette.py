...
def test_frmarticulos_ui_listwidget_properties():
    ...
    for widget in root.iter("widget"):
        if (
            widget.attrib.get("class") == "QListWidget"
            and widget.attrib.get("name") == "listWidget"
        ):
            list_widget = widget
            break
    ...
    properties = {
        (
            p.find("string").text
            if p.find("string") is not None
            else p.find("number").text if p.find("number") is not None else None
        ): p
        for p in list_widget.findall("property")
    }
