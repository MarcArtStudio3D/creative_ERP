...
def main():
    ...
    logging.getLogger(__name__).info(
        "1. Click en 'Añadir sección' para crear una nueva sección"
    )
    logging.getLogger(__name__).info(
        "2. Selecciona una sección y click en 'Añadir familia'"
    )
    logging.getLogger(__name__).info(
        "3. Selecciona una familia y click en 'Añadir subfamilia'"
    )
    logging.getLogger(__name__).info(
        "4. Usa los botones 'Editar' y 'Borrar' para modificar/eliminar"
    )
    logging.getLogger(__name__).info(
        "5. El árbol superior muestra la jerarquía seleccionada"
    )
