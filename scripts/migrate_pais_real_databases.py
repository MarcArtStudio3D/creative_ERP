...
def migrate_mariadb(host, port, user, password, database):
    ...
    try:
        ...
        if has_id_pais and not has_pais:
            ...
            cursor.execute("UPDATE clientes SET pais = CASE id_pais WHEN 1 THEN 'España' WHEN 7 THEN 'Francia' ELSE 'España' END")
        ...
        if cursor.fetchone():
            ...
            if alt_has_id_pais and not alt_has_pais:
                cursor.execute("ALTER TABLE direcciones_alternativas ADD COLUMN pais VARCHAR(100)")
                cursor.execute("UPDATE direcciones_alternativas SET pais = CASE id_pais WHEN 1 THEN 'España' WHEN 7 THEN 'Francia' ELSE 'España' END")
                cursor.execute("ALTER TABLE direcciones_alternativas DROP COLUMN id_pais")
...
def migrate_postgresql(host, port, user, password, database):
    ...
    try:
        ...
        cursor.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'direcciones_alternativas')")
        if cursor.fetchone()[0]:
            cursor.execute("SELECT column_name FROM information_schema.columns WHERE table_name = 'direcciones_alternativas'")
...
def migrate_sqlite(db_path):
    ...
    try:
        ...
        # Migrar direcciones_alternativas si existe
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='direcciones_alternativas'")
        if cursor.fetchone():
            ...
            if alt_has_id_pais and not alt_has_pais:
                ...
                cursor.execute("ALTER TABLE direcciones_alternativas_new DROP COLUMN id_pais")
                cursor.execute("DROP TABLE direcciones_alternativas")
                cursor.execute("ALTER TABLE direcciones_alternativas_new RENAME TO direcciones_alternativas")
