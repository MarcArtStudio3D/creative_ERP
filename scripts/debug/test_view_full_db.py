...
def test_view_full_database_connection():
    ...
    try:
        ...
        if not target_company and empresas:
            ...
            print(
                "⚠️  No se encontró BD artstudio3d, usando primera empresa disponible para la prueba"
            )
        ...
        print(
            f"Using company: {company_id} (DB: {target_company.get('database_name')})"
        )
        ...
        try:
            ...
            if len(clientes) > 0:
                ...
                print(
                    f"   Ejemplo: {primer_cliente.nombre_fiscal} (ID: {primer_cliente.id})"
                )
