...
def test_save_familia():
    ...
    art = session.execute(text("SELECT id, codigo, id_familia FROM articulos WHERE codigo IN ('ART001','ART002','ART003') LIMIT 1")).fetchone()
    if not art:
    ...
    familias = session.execute(text("SELECT id, codigo, familia FROM familias ORDER BY id LIMIT 2")).fetchall()
    ...
    print(f"🔍 Seleccionando familia: {familia_codigo} - {familia_nombre} (ID {familia_id})")

    success = controller.set_familia_from_lookup(familia_id, familia_codigo, familia_nombre)
    if not success:
