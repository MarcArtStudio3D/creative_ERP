...
def test_save_seccion():
    ...
    article_result = session.execute(
        text("SELECT id FROM articulos WHERE codigo = 'ART001'")
    ).fetchone()
    ...
    success = controller.set_seccion_from_lookup(
        seccion_id, seccion_codigo, seccion_nombre
    )
    if not success:
    ...
    print(
        f"📊 Estado después de lookup - id_seccion: {updated_article.get('id_seccion')}"
    )
    ...
    if db_result:
        ...
        if db_result[2] == seccion_id:
            ...
            print(
                f"❌ ERROR: Sección no coincide (esperado: {seccion_id}, obtenido: {db_result[2]})"
            )
    else:
