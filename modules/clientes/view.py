...
class ClientesView(QWidget):
    ...
    def __init__(
        self, parent=None, preserve_styles=None, custom_styles=None, session=None
    ):
        """
    ...
    def conectar_senales(self):
            ...
            if hasattr(tabla, "itemDoubleClicked"):
                try:
                    tabla.itemDoubleClicked.connect(
                        lambda *args: self.abrir_ficha_cliente()
                    )
                except Exception:
                    pass
                ...
                try:
                    tabla.cellDoubleClicked.connect(
                        lambda r, c: self.abrir_ficha_cliente()
                    )
                except Exception:
                    pass
        ...
        for name, handler in (
            ("btnGuardar", self.guardar_cliente),
            ("btnDeshacer", self.deshacer_cambios),
        ):
            w = self._get_widget(name)
            if w is not None and hasattr(w, "clicked"):
                w.clicked.connect(handler)
        ...
        if lista_direcciones is not None:
            try:
                lista_direcciones.doubleClicked.connect(
                    self.editar_direccion_alternativa
                )
                # Conectar cambio de selección para mostrar datos (solo en modo no edición)
                lista_direcciones.currentChanged.connect(
                    self.mostrar_direccion_alternativa
                )
            except Exception:
                pass
        ...
        if (
            not hasattr(self, "_validations_connected")
            or not self._validations_connected
        ):
            try:
                from PySide6.QtWidgets import (
                    QComboBox,
                ...
                if w_cp_alt is not None and isinstance(w_cp_alt, QLineEdit):
                    try:
                        w_cp_alt.returnPressed.connect(
                            self._handle_postal_code_alternativa_search
                        )
                    except Exception:
                        pass
                ...
                if w_pob_alt is not None and isinstance(w_pob_alt, QLineEdit):
                    try:
                        w_pob_alt.returnPressed.connect(
                            self._handle_poblacion_alternativa_change
                        )
                    except Exception:
                        pass
    ...
    def abrir_tipo_cliente(self):
        ...
        if not self.cliente_actual:
            show_warning(
                self,
                self.tr("Aviso"),
                self.tr("Debe guardar el cliente antes de asignar tipos."),
            )
            return
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                if (
                    hasattr(dialog, "resultado_seleccion")
                    and dialog.resultado_seleccion
                ):
                    ...
                    if tipo_id:
                        ...
                        try:
                            if self.cliente_actual and hasattr(
    ...
    def _has_validation_error(self, widget) -> bool:
        ...
        return bool(
            tooltip and ("error" in tooltip.lower() or "inválido" in tooltip.lower())
        )
        ...
        try:
            ...
            try:
                ...
                cols = (
                    m.columnCount()
                    if m is not None
                    else (tabla.columnCount() if hasattr(tabla, "columnCount") else 0)
                )
            except Exception:
                try:
    ...
    def _get_str(self, obj, attr: str) -> str:
        ...
        Maneja tanto objetos (con getattr) como diccionarios (con get).
        """
        try:
    ...
    def _get_value(self, obj, attr: str):
        """Obtiene un valor de un objeto o diccionario de forma segura.

        Devuelve el valor tal cual (puede ser None, int, str, etc.).
        """
        try:
            if isinstance(obj, dict):
    ...
    def _validar_campos(self) -> tuple[bool, list]:
        ...
        if hasattr(self.ui, "txtcodigo_cliente"):
            codigo = (
                self.ui.txtcodigo_cliente.text()
                if hasattr(self.ui.txtcodigo_cliente, "text")
                else ""
            )
        elif getattr(self.ui, "txtcodigo_cliente", None) is not None:
            # getattr fallback handled above
            codigo = (
                getattr(self.ui, "txtcodigo_cliente").text()
                if hasattr(getattr(self.ui, "txtcodigo_cliente"), "text")
                else ""
            )
        ...
        if not (v_nombre and v_nombre.strip()) and not (
            v_nombre_fiscal and v_nombre_fiscal.strip()
        ):
            errores.append(
                self.tr("Debe introducir el nombre o el nombre fiscal del cliente.")
            )
        ...
        if (
            getattr(self.ui, "txtiban", None) is not None
            or getattr(self.ui, "txtCuentaIBAN", None) is not None
        ):
            w = getattr(self.ui, "txtiban", None) or getattr(
                self.ui, "txtCuentaIBAN", None
            )
            ...
            if w is not None:
                try:
                    ...
                    if val is not None and not (0 <= int(val) <= 31):
                        errores.append(
                            self.tr("Día de pago {} fuera de rango 0-31.").format(dname)
                        )
                except Exception:
                    errores.append(
                        self.tr("Día de pago {} no es un número válido.").format(dname)
                    )
    ...
    def filter_records(self, search_text: str, order_by: str, order_mode: str):
        ...
        try:
            ...
            if order_by == "Código":
                clientes.sort(
                    key=lambda c: c.codigo_cliente or "", reverse=(order_mode == "Z-A")
                )
            elif order_by == "Fecha":
                clientes.sort(
                    key=lambda c: c.fecha_alta or date(1900, 1, 1),
                    reverse=(order_mode == "Z-A"),
                )
            else:  # "Nombre Fiscal" por defecto
                clientes.sort(
                    key=lambda c: (
                        c.nombre_fiscal or c.nombre_completo() or ""
                    ).lower(),
                    reverse=(order_mode == "Z-A"),
                )
            ...
            if isinstance(tabla, QTableWidget):
                ...
                for row, cliente in enumerate(clientes):
                    ...
                    # Nombre Fiscal
                    nombre_fiscal = self._get_str(cliente, "nombre_fiscal") or (
            ...
            show_warning(
                self,
                self.tr("Error"),
                self.tr("Error al filtrar clientes: {}").format(str(e)),
            )
    ...
    def abrir_ficha_cliente(self):
        ...
        try:
            # Obtener tabla
            tabla = getattr(self.ui, "tabla_busquedas", None) or getattr(
                self.ui, "tabla_clientes", None
            )
            if not tabla:
                return
            ...
            try:
                ...
                show_warning(
                    self, self.tr("Error"), f"Error en controller: {controller_error}"
                )
                return
            ...
            try:
                ...
                print(
                    f"Advertencia: No se pudieron cargar todos los datos del formulario: {e}"
                )
            ...
            if hasattr(self.ui, "stackedWidget"):
                ...
                print("ERROR: stackedWidget no existe en la UI")
    ...
    def cargar_datos_en_formulario(self, cliente: Cliente):
        ...
        if hasattr(self.ui, "txtnombre_comercial"):
            self.ui.txtnombre_comercial.setText(
                self._get_str(cliente, "nombre_comercial")
            )
            ...
            if hasattr(self.ui, "cboDivisa"):
                ...
                for i in range(self.ui.cboDivisa.count()):
                    ...
                    if (
                        ...
                        and id_divisa is not None
                        and int(itemdata) == id_divisa
                    ):
        ...
        try:
            if hasattr(self.ui, "cboforma_pago"):
                id_forma_pago = self._get_value(cliente, "id_forma_pago")
                for i in range(self.ui.cboforma_pago.count()):
                    itemdata = self.ui.cboforma_pago.itemData(i)
                    if (
                        itemdata is not None
                        and id_forma_pago is not None
                        and int(itemdata) == id_forma_pago
                    ):
        ...
        try:
            if hasattr(self.ui, "cbotarifa_cliente"):
                id_tarifa = self._get_value(cliente, "id_tarifa")
                for i in range(self.ui.cbotarifa_cliente.count()):
                    itemdata = self.ui.cbotarifa_cliente.itemData(i)
                    if (
                        itemdata is not None
                        and id_tarifa is not None
                        and int(itemdata) == id_tarifa
                    ):
        ...
        try:
            if hasattr(self.ui, "cboagente"):
                id_agente = self._get_value(cliente, "id_agente")
                for i in range(self.ui.cboagente.count()):
                    itemdata = self.ui.cboagente.itemData(i)
                    if (
                        itemdata is not None
                        and id_agente is not None
                        and int(itemdata) == id_agente
                    ):
                ...
                for i in range(self.ui.cboPais.count()):
                    ...
                    if itemdata is not None:
                        ...
                        if isinstance(itemdata, dict) and (
                            itemdata.get("es") == pais_valor
                            or itemdata.get("fr") == pais_valor
                        ):
    ...
    def cargar_direcciones_alternativas(self, id_cliente: int):
        ...
        if (
            direcciones
            and len(direcciones) > 0
            and not getattr(self, "_modo_edicion_direccion", False)
        ):
    ...
    def cargar_estadisticas(self, id_cliente: int):
        ...
        for mes, nombre_campo in meses_map.items():
            if hasattr(self.ui, nombre_campo):
                ...
                getattr(self.ui, nombre_campo).setText(
                    format_decimal_value(importe, decimals, use_comma=True)
                )
    ...
    def nuevo_cliente(self):
        ...
        if hasattr(self.ui, "tabwidget") and self.ui.tabwidget.count() == 0:
            ...
            tab_texts = [
                ...
                "",
                "",
            ]  # Los textos se definen en retranslateUi
    ...
    def editar_cliente(self):
        ...
        if (
            ...
            and hasattr(self.ui, "stackedWidget")
        ...
        if not selection.hasSelection():
            show_warning(
                self, self.tr("Aviso"), self.tr("Seleccione un cliente para editar")
            )
            return
    ...
    def borrar_cliente(self):
        ...
        if not selection.hasSelection():
            show_warning(
                self, self.tr("Aviso"), self.tr("Seleccione un cliente para borrar")
            )
            return
        ...
        reply = show_question(
            ...
            self.tr("¿Está seguro de que desea borrar el cliente '{}'?").format(
                cliente.nombre_completo()
            ),
    ...
    def guardar_cliente(self):
        ...
        try:
            ...
            txt_map = [
                ("codigo_cliente", "txtcodigo_cliente"),
                ("cif_nif_siren", "txtcif_nif"),
                ("nombre", "txtnombre"),
                ("apellido1", "txtPrimerApellido"),
                ("apellido2", "txtSegundoApellido"),
                ("nombre_fiscal", "txtnombre_fiscal"),
                ("nombre_comercial", "txtnombre_comercial"),
                ("direccion1", "txtdireccion1"),
                ("direccion2", "txtdireccion2"),
                ("cp", "txtcp"),
                ...
                ("web", "txtweb"),
                ("siret", "txtSiret"),
                ("cif_vies", "txtCifIntracomunitario"),
                ("persona_contacto", "txtpersona_contacto"),
                ("entidad_bancaria", "txtentidad_bancaria"),
                ("oficina_bancaria", "txtoficina_bancaria"),
                ("dc", "txtdc"),
                ("cuenta_corriente", "txtcuenta_corriente"),
                ("acceso_web", "txtacceso_web"),
                ("password_web", "txtpassword_web"),
                ("cuenta_contable", "txtcuenta_contable"),
                ("cuenta_iva_repercutido", "txtcuenta_iva_repercutido"),
                ("cuenta_deudas", "txtcuenta_deudas"),
                ("cuenta_cobros", "txtcuenta_cobros"),
                ("visa_distancia1", "txtvisa_distancia1"),
                ("visa_distancia2", "txtvisa_distancia2"),
                ("comentarios", "txtcomentarios"),
                ("comentario_bloqueo", "txtcomentario_bloqueo"),
                ("observaciones", "txtObservaciones"),
            ]
            date_map = [
                ("fecha_alta", "txtfecha_alta"),
                ("fecha_nacimiento", "txtfecha_nacimiento"),
                ("fecha_ultima_compra", "txtfecha_ultima_compra"),
            ]
            int_map = [
                ("dia_pago1", "txtdia_pago1"),
                ("dia_pago2", "txtdia_pago2"),
                ("visa1_caduca_mes", "txtvisa1_caduca_mes"),
                ("visa2_caduca_mes", "txtvisa2_caduca_mes"),
                ("visa1_caduca_ano", "txtvisa1_caduca_ano"),
                ("visa2_caduca_ano", "txtvisa2_caduca_ano"),
                ("visa1_cod_valid", "txtvisa1_cod_valid"),
                ("visa2_cod_valid", "txtvisa2_cod_valid"),
            ]
            float_map = [
                ("acumulado_ventas", "txtimporteAcumulado"),
                ("ventas_ejercicio", "txtventas_ejercicio"),
                ("riesgo_maximo", "txtrRiesgoPermitido"),
                ("deuda_actual", "txtdeuda_actual"),
                ("porc_dto_cliente", "txtporc_dto_cliente"),
                ("importe_a_cuenta", "txtimporte_a_cuenta"),
                ("vales", "txtvales"),
            ]
            bool_map = [
                ("recargo_equivalencia", "chkrecargo_equivalencia"),
                ("irpf", "chkClienteEmpresa"),
                ("bloqueado", "chklBloqueoCliente"),
            ]
            cbo_map = [
                ("id_idioma_documentos", "cboidiomaDocumentos"),
                ("pais", "cboPais"),
                ("id_divisa", "cboDivisa"),
                ("id_forma_pago", "cboforma_pago"),
                ("id_tarifa", "cbotarifa_cliente"),
                ("id_agente", "cboagente"),
                ("id_transportista", "cbotransportista"),
            ]
            ...
            if "transaction has been rolled back" in error_msg:
                ...
                show_critical(
                    ...
                    self.tr(
                        "Error al guardar cliente. La base de datos ha rechazado la operación.\n\n"
                        "Posibles causas:\n"
        ...
        try:
            if not error_message:
                ...
                try:
                    ...
                    widget.setStyleSheet(
                        "border: 1px solid #ff4d4f; border-radius: 2px;"
                    )
                except Exception:
    ...
    def _maybe_fill_nombre_fiscal(self):
        ...
        try:
            ...
            a1 = (
                w_ap1.text().strip()
                if (w_ap1 is not None and hasattr(w_ap1, "text"))
                else ""
            )
            a2 = (
                w_ap2.text().strip()
                if (w_ap2 is not None and hasattr(w_ap2, "text"))
                else ""
            )
            nombre = (
                w_nombre.text().strip()
                if (w_nombre is not None and hasattr(w_nombre, "text"))
                else ""
            )

            # If target field not present, nothing to do
            if (
                w_nombre_fiscal is None
                or not hasattr(w_nombre_fiscal, "text")
                or not hasattr(w_nombre_fiscal, "setText")
            ):
                return
    ...
    def _validate_and_apply(self, widget_name: str):
            ...
            if txt and not self._is_valid_nif_cif(txt):
                ...
                if txt and txt.strip() and getattr(self, "_modo_edicion", False):
                    ...
                    if cliente_existente:
                        ...
                        QTimer.singleShot(
                            100, lambda: self._handle_duplicate_cif(cliente_existente)
                        )
        ...
        # Nombre / Nombre fiscal - both interplay
            ...
            if (not v1 or not v1.strip()) and (not v2 or not v2.strip()):
                # mark both invalid
                self._apply_widget_valid_state(
                    w1, "Se requiere un nombre o nombre fiscal"
                )
                self._apply_widget_valid_state(
                    w2, "Se requiere un nombre o nombre fiscal"
                )
            else:
                ...
                if w is None:
                    ...
                    val = (
                        ...
                        if hasattr(w, "value")
                            ...
                            int(w.text()) if (hasattr(w, "text") and w.text()) else None
                        )
                    )
            ...
            if isinstance(tabla, QTableWidget):
                ...
                for r in range(rows):
                    ...
                    try:
                        ...
                        val = (
                            data_get(Qt.ItemDataRole.UserRole)
                            if callable(data_get)
                            else None
                        )
                    except Exception:
                        val = None
                    if val == cid:
                        ...
                        print(
                            f"[DEBUG] _reseleccionar_cliente_en_tabla: Selected row {r} for client ID {cid}"
                        )
                        break
    ...
    def _handle_postal_code_search(self):
        ...
        try:
            ...
            results, db_path, db_config = self.controller.buscar_poblacion_por_cp(
                cp, pais="Francia"
            )
    ...
    def _handle_poblacion_search(self):
        ...
        try:
            # Delegar búsqueda al controller (retorna tupla)
            results, db_path, db_config = (
                self.controller.buscar_cp_por_poblacion_simple(
                    poblacion, pais="Francia"
                )
            )
            ...
            if len(results) == 1:
                ...
                country_db = QSqlDatabase.addDatabase(
                    "QSQLITE", "france_connection_poblacion_search"
                )
                country_db.setDatabaseName(db_path)
        ...
        try:
            ...
            for i in range(combo.count()):
                ...
                if itemdata and isinstance(itemdata, dict):
                    ...
                    if (
                        pais_es.lower() == pais_empresa.lower()
                        or pais_fr.lower() == pais_empresa.lower()
                    ):
                        combo.setCurrentIndex(i)
                        return
    ...
    def mostrar_dialogo_poblacion_alternativa(
    ...
    def anadir_direccion_alternativa(self):
        ...
        if not self.cliente_actual:
            show_warning(
                self, self.tr("Error"), self.tr("Primero debe seleccionar un cliente.")
            )
            return
    ...
    def editar_direccion_alternativa(self):
        ...
        if not current_index.isValid():
            show_info(
                self,
                self.tr("Seleccionar dirección"),
                self.tr("Por favor, seleccione una dirección de la lista."),
            )
            return
        ...
        if not direccion:
            show_warning(
                self, self.tr("Error"), self.tr("No se pudo cargar la dirección.")
            )
            return
    ...
    def borrar_direccion_alternativa(self):
        ...
        if not current_index.isValid():
            show_info(
                self,
                self.tr("Seleccionar dirección"),
                self.tr("Por favor, seleccione una dirección de la lista para borrar."),
            )
            return
        ...
        try:
            if self.repository.eliminar_direccion(id_direccion):
                show_info(
                    self,
                    self.tr("Éxito"),
                    self.tr("Dirección eliminada correctamente."),
                )
                # Recargar lista
                if self.cliente_actual:
            ...
            show_critical(
                self,
                self.tr("Error"),
                self.tr("Error al eliminar la dirección: {}").format(str(e)),
            )
        ...
        try:
            ...
            if self._direccion_actual:
                ...
                show_info(
                    self,
                    self.tr("Éxito"),
                    self.tr("Dirección actualizada correctamente."),
                )
            else:
                # Crear nueva
                ...
                show_info(
                    self, self.tr("Éxito"), self.tr("Dirección creada correctamente.")
                )
            ...
            show_critical(
                self,
                self.tr("Error"),
                self.tr("Error al guardar la dirección: {}").format(str(e)),
            )
    ...
    def limpiar_campos_direccion_alternativa(self):
        ...
        campos = [
            ...
            "txtcpPoblacionAlternativa",
            "txtpoblacionAlternativa",
            "txtdireccion1Alternativa1",
            "txtdireccion1Alternativa2",
            "txtprovinciaAlternativa",
            "txtemail_alternativa",
            "txtcomentarios_alternativa",
        ]
