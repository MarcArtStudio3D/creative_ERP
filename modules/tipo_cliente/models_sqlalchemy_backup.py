...
class TipoCliente(Base):
    ...
    subtipos: Mapped[List["TipoSubCliente"]] = relationship(
        "TipoSubCliente", back_populates="tipo", cascade="all, delete-orphan"
    )
...
class TipoSubCliente(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    id_tipocliente: Mapped[int] = mapped_column(
    ...
    tipo: Mapped["TipoCliente"] = relationship("TipoCliente", back_populates="subtipos")
