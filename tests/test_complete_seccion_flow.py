...
def test_complete_flow():
    ...
    article_result = session.execute(
        text("SELECT id FROM articulos WHERE codigo = 'ART003'")
    ).fetchone()
    ...
    print(
        f"   Nombre sección: {controller.get_seccion_name(initial_seccion) if initial_seccion else 'Sin sección'}"
    )
    ...
    success = controller.set_seccion_from_lookup(
        new_seccion_id, new_seccion_codigo, new_seccion_nombre
    )
    if not success:
    ...
    form_data = {
        ...
        "descripcion_reducida": article.get(
