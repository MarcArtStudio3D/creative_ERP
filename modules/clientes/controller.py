...
class ClientesController(QObject):
    ...
    def __init__(self, session: Session, parent=None):
        ...
        self.model.setHorizontalHeaderLabels(["Código", "NIF/CIF", "Nombre Fiscal", "Teléfono", "Email"])
        ...
        try:
            ...
            for cliente in clientes:
                items = [
                    ...
                    QStandardItem(getattr(cliente, 'nombre_fiscal', '') or 
                                (cliente.nombre_completo() if hasattr(cliente, 'nombre_completo') else '')),
    ...
    def obtener_paises(self) -> List[tuple]:
        ...
        try:
            ...
            if not os.path.exists(db_path):
                self.error_occurred.emit(f"Base de datos de países no encontrada: {db_path}")
                return []
    ...
    def buscar_cp_por_poblacion_alternativa(self, poblacion: str, pais: str = "Francia"):
        ...
        try:
            ...
            if len(results) == 1:
                ...
                # Multiple results - show dialog through view
                self._show_selection_dialog_alternativa(results, poblacion, is_cp_search=False)
            else:
                pass
        except Exception as e:
            self.error_occurred.emit(f"Error al buscar CP alternativo: {e}")

    def _show_selection_dialog_alternativa(self, results, search_term, is_cp_search=True):
        """Delega mostrar diálogo de selección a la vista"""
        try:
            # La vista debe implementar este método para mostrar el diálogo
            if hasattr(self, '_view_reference'):
                self._view_reference.mostrar_dialogo_poblacion_alternativa(results, search_term, is_cp_search)
        except Exception as e:
