...
def test_ensure_uses_company_db(monkeypatch, qapp):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"has_company": True, "company_id": 42, "database_name": "company_42"},
    )
    ...
    monkeypatch.setattr(
        "modules.articulos.view_tarifas_base.TarifaTipoController", _DummyController
    )
    ...
    monkeypatch.setattr(
        "modules.articulos.view_tarifas_base.set_current_database", fake_set
    )
...
def test_ensure_no_change_when_not_main(monkeypatch, qapp):
    ...
    monkeypatch.setattr(
        "core.company_manager.get_current_company_context",
        lambda: {"has_company": True, "company_id": 88, "database_name": "company_88"},
    )
...
def test_ensure_no_company_selected_leaves_main(monkeypatch, qapp):
    ...
    monkeypatch.setattr(
        "modules.articulos.view_tarifas_base.TarifaTipoController", lambda *a, **k: None
    )
