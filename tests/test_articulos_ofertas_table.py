...
from modules.articulos.view import ArticulosView, OffersTableModel
...
def test_offers_table_shows_active_indicator_and_description():
    ...
    row = session.execute(
        text("SELECT id FROM articulos LIMIT 1")
    ).fetchone()
    ...
    row2 = session.execute(text("SELECT id FROM articulos WHERE id != :id LIMIT 1"), {"id": art_id}).fetchone()
    if row2:
        other = row2[0]
