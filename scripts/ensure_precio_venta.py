...
def select_targets(
    available, requested_list=None, include_companies=None, allow_main=False
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Propaga columna precio_venta a las DBs configuradas"
    )
    parser.add_argument(
        "--apply", action="store_true", help="Aplicar los cambios (por defecto dry-run)"
    )
    parser.add_argument(
        "--dbs",
        help="Lista separada por comas de DBs a afectar (ej: artstudio3d,company_1)",
    )
    parser.add_argument(
        "--all-companies",
        action="store_true",
        help="Detectar y añadir empresas (company_*) desde la BD principal",
    )
    parser.add_argument(
        "--allow-main",
        action="store_true",
        help="Permite incluir la base de datos `main` en las operaciones (por defecto está excluida)",
    )
    ...
    targets = select_targets(
        list_available_databases(),
        requested_list=[d.strip() for d in args.dbs.split(",")] if args.dbs else None,
    ...
    if args.all_companies:
        ...
        try:
            ...
            targets = select_targets(
                discovered, include_companies=company_keys, allow_main=args.allow_main
            )
    ...
    for db_key in targets:
        try:
            ...
            dialect = (
                "sqlite"
                if "sqlite" in db_url
                else ("mysql" if "mysql" in db_url or "mariadb" in db_url else "other")
            )
            ...
            if not args.apply:
                print(
                    "     (dry-run) no se aplicarán cambios. Usa --apply para añadir la columna."
                )
