...
def collect_strings_from_file(path: Path) -> Dict[str, Set[str]]:
    ...
    class ContextVisitor(ast.NodeVisitor):
        ...
        def visit_Call(self, node: ast.Call):
            ...
            if isinstance(func, ast.Attribute) and getattr(func.value, 'id', '') == 'QCoreApplication' and func.attr == 'translate':
                if len(node.args) >= 2:
            ...
            if func_name in ('show_info', 'show_warning', 'show_critical'):
                ...
                strs = [literal_str(a) for a in node.args if isinstance(a, ast.Constant) and isinstance(a.value, str)]
                if strs:
...
def merge_into_ts(ts_path: Path, source_map: Dict[str, Set[str]], dry_run: bool = False) -> Tuple[int, int]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sync translation .ts with source strings and optionally run lupdate')
    parser.add_argument('--dry-run', action='store_true', help='Do not modify .ts, only report')
    parser.add_argument('--run-lupdate', action='store_true', help='Run pyside6-lupdate/lupdate before scanning')
    parser.add_argument('--apply', action='store_true', help='Write changes into .ts files (opposite of dry-run)')
