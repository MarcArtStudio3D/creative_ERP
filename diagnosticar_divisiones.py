...
def diagnosticar():
    ...
    if "divisiones_almacen" in AVAILABLE_MODULES:
        ...
        logging.getLogger(__name__).debug(
            f"      - Permisos requeridos: {[p.value for p in mod.required_permissions]}"
        )
        logging.getLogger(__name__).debug(f"      - Habilitado: {mod.enabled}")
    else:
        logging.getLogger(__name__).warning(
            "   ❌ Módulo NO encontrado en AVAILABLE_MODULES"
        )
        return False
    ...
    if "divisiones_almacen" in available_ids:
        ...
        logging.getLogger(__name__).debug(
            f"   Available modules: {', '.join(available_ids[:10])}..."
        )
    ...
    logging.getLogger(__name__).debug(
        f"   Almacen modules found: {len(almacen_modules)}"
    )
    ...
    view_path = os.path.join(
        os.path.dirname(__file__), "..", "modules", "divisiones_almacen", "view.py"
    )
    if os.path.exists(view_path):
        logging.getLogger(__name__).info(f"   ✅ Archivo view.py existe: {view_path}")
    else:
        logging.getLogger(__name__).warning(
            f"   ❌ Archivo view.py NO existe: {view_path}"
        )

    init_path = os.path.join(
        os.path.dirname(__file__), "..", "modules", "divisiones_almacen", "__init__.py"
    )
    if os.path.exists(init_path):
        logging.getLogger(__name__).info(
            f"   ✅ Archivo __init__.py existe: {init_path}"
        )
    else:
        logging.getLogger(__name__).warning(
            f"   ❌ Archivo __init__.py NO existe: {init_path}"
        )
    ...
    try:
        logging.getLogger(__name__).info(
            "   ✅ Importación exitosa de DivisionesAlmacenView"
        )
    except Exception:
        logging.getLogger(__name__).exception(
            "   ❌ Error al importar DivisionesAlmacenView"
        )
    ...
    logging.getLogger(__name__).info("\nSOLUTION:")
    logging.getLogger(__name__).info(
        "   1. Asegúrate de haber reiniciado completamente la aplicación"
    )
    logging.getLogger(__name__).info(
        "   2. Verifica que iniciaste sesión con el usuario 'admin'"
    )
    logging.getLogger(__name__).info(
        "   3. Ve a la categoría 'Almacén' en la barra lateral"
    )
    logging.getLogger(__name__).info(
        "   4. You should see the 'Secciones Almacén' button"
    )
