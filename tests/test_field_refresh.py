...
def test_field_refresh():
    ...
    if result.scalar() == 0:
        ...
        session.execute(
            text("INSERT INTO secciones (codigo, seccion) VALUES ('S1', 'GENERAL')")
        )
        session.execute(
            text("INSERT INTO secciones (codigo, seccion) VALUES ('S2', 'PREMIUM')")
        )
        session.commit()
    ...
    articles = session.execute(
        text("SELECT id, codigo, id_seccion FROM articulos ORDER BY id LIMIT 2")
    ).fetchall()
    ...
    if len(articles) >= 2:
        ...
        seccion1_name = (
            controller.get_seccion_name(seccion1_id) if seccion1_id else "Sin sección"
        )
        ...
        seccion2_name = (
            controller.get_seccion_name(seccion2_id) if seccion2_id else "Sin sección"
        )
        print(f"   -> Sección: {seccion2_name} (ID: {seccion2_id})")

        if seccion1_id != seccion2_id:
            print(
                "✅ Los artículos tienen secciones diferentes - el campo debe actualizarse"
            )
        else:
        ...
        controller.load_by_id(articles[0][0])
        print(
            f"Artículo actual: {controller.get_current_article().get('codigo')} - Sección: {controller.get_seccion_name(controller.get_current_article().get('id_seccion'))}"
        )
    ...
    session.close()
    print(
        "🧪 Test completado - si ves diferentes secciones, el refresh debería funcionar"
    )
