...
def test_subfamilias_filtered_by_familia():
    ...
    subs = session.execute(text("SELECT id FROM subfamilias WHERE id_familia = :fid LIMIT 1"), {'fid': fam_id}).fetchall()
    if not subs:
    ...
    for item in repo_res:
        ...
        row = session.execute(text("SELECT id_familia FROM subfamilias WHERE id = :id"), {'id': item['id']}).fetchone()
        assert row[0] == fam_id
