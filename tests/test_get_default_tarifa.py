...
def test_get_default_tarifa_reads_main_empresas(monkeypatch):
    ...
    session.execute(
        text("INSERT INTO empresas (id, group_id, codigo_empresa, nombre_fiscal, cif_nif, tarifa_predeterminada, activa) VALUES (:id, :group_id, :codigo, :nombre, :cif, :tarifa, 1)"),
        {'id': company_id, 'group_id': 1, 'codigo': 'C-TEST', 'nombre': 'Empresa Test', 'cif': 'TESTCIF', 'tarifa': '777'}
    )
    session.commit()

    # Monkeypatch company context to pretend this company is selected
    monkeypatch.setattr('core.company_manager.get_current_company_context', lambda: {'has_company': True, 'company_id': company_id})
