...
DEFAULT_TABLES = [
    'articulos', 'articulos_imagenes', 'articulos_ofertas',
    'clientes', 'familias', 'subfamilias', 'tarifas', 'kits',
    'proveedores_frecuentes', 'direcciones_alternativas', 'deudas_clientes',
    'estadisticas_clientes_mes', 'historial_clientes'
]
...
def main():
    parser = argparse.ArgumentParser(description='Safe drop tables in main DB (dry-run by default)')
    parser.add_argument('--tables', help='Comma-separated list of tables to drop (default: common module tables)')
    parser.add_argument('--apply', action='store_true', help='Show SQL and (with --confirm) execute drops')
    parser.add_argument('--confirm', help="Confirmation token required to run destructive drops. Type 'DELETE_MAIN' to confirm")
    parser.add_argument('--out-file', help='Optional path to write the generated SQL for manual execution')
